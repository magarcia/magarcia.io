...
Last week I wrote about [using npx bun to write Claude Code skills](/writing-powerful-claude-code-skills-with-npx-bun) with third-party dependencies. The approach worked for simple cases—self-contained scripts with auto-installing packages, no build step. But after using it in real environments, I discovered a significant problem.
...
Credit for the solution goes to [J Edward Wynia](https://www.threads.net/@jwynia/post/DTgiB62DaxN), who pointed me toward Deno in response to that article. I forget why I skipped Deno initially—probably because Bun's syntax looked cleaner—but the suggestion was right.
...
The structure follows the same pattern from the [original article](/writing-powerful-claude-code-skills-with-npx-bun)—a `SKILL.md` pointing to executable scripts. The only changes are the shebang and Deno-specific APIs:
