---
title: "Construyendo granola-cli: Notas de Reuniones con IA en tu Terminal"
date: "2025-12-22"
spoiler:
  granola-cli trae tus notas de reuniones de Granola.ai a la línea de comandos.
  Consulta reuniones con Claude Code, busca en transcripciones, exporta a
  JSON/TOON, y canaliza tareas a tus flujos de trabajo. Construido con
  almacenamiento seguro en keychain y filtrado de fechas en lenguaje natural.
tags:
  - typescript
  - node-js
  - cli
...
¿Y si pudieras consultar tu historial de reuniones como una base de datos?
**granola-cli** trae tus notas de reuniones de Granola.ai a la línea de
comandos—busca en transcripciones, exporta a JSON, y canaliza tareas
directamente a Claude Code o tu gestor de tareas.
...
Copiar y pegar desde una interfaz web no bastaba. Encontré
...
El trabajo base existía. Una CLI adecuada para uso diario, no.
...
un formato diseñado para consumo de LLMs. TOON entrega los mismos datos
estructurados que JSON usando **40% menos tokens**.

Cuando canalizas datos de reuniones a Claude u otro modelo, cada token ahorrado
significa más contexto para tu pregunta. TOON determina si cabe una reunión o
tres en tu ventana de contexto.
...
La CLI almacena credenciales mediante **cross-keychain** en el gestor de
...
Granola usa tokens de actualización de un solo uso: cada token funciona una vez
antes de invalidarse. Esto mejora la seguridad pero crea una condición de
carrera: si dos procesos CLI actualizan simultáneamente, uno obtiene un token
válido mientras el otro desperdicia el token de actualización y falla.

La CLI resuelve esto con bloqueo basado en archivos. Antes de actualizar, el
proceso adquiere un bloqueo exclusivo en un archivo temporal. Si otro proceso ya
está actualizando, el segundo espera (timeout de 30 segundos) en lugar de
competir. El bloqueo se libera inmediatamente después de completar la
actualización, por lo que las invocaciones paralelas de la CLI funcionan sin
problemas: se turnan cuando es necesario.
...
un lunes?) y devuelve timestamps UTC que coinciden con las expectativas de la
API de Granola. El caso común—"muéstrame las reuniones de ayer"—se convierte en
un solo flag intuitivo.
...
- [Almacenamiento Seguro de Secretos en Node.js con cross-keychain](/cross-platform-secret-storage-with-cross-keychain) -
  La librería que granola-cli usa para almacenamiento seguro de credenciales
- [Pedir a la IA que construya la herramienta en lugar de hacer la tarea](/asking-ai-to-build-the-tool-instead-of-doing-the-task) -
  Cómo abordo el desarrollo asistido por IA
