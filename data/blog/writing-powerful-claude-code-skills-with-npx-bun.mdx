...
updated: "2026-01-15"
spoiler:
  How to write Claude Code skills that use third-party npm packages without a
  build step. Using npx bun gives you auto-installing dependencies, native
  TypeScript, and clean version pinning—the JavaScript equivalent of Python's uv
  run.
...
> `node_modules` detection can break auto-install in unexpected situations. I
...
I wanted to build more complex
[Claude Code](https://docs.anthropic.com/en/docs/claude-code/skills) skills.
Claude Code is Anthropic's CLI for agentic coding—and skills let you extend it
with custom instructions and executable scripts. Not just markdown, but scripts
that use third-party libraries: parsing CSV files, manipulating spreadsheets,
calling APIs with proper HTTP clients.

The problem: skills have no build step. You can't just
`import lodash from 'lodash'` and expect it to work. The script runs in a fresh
environment with no `node_modules` folder.

## What I Wanted: uv run for JavaScript

Python solves this elegantly with [uv](https://docs.astral.sh/uv/). You declare
dependencies inline using [PEP 723](https://peps.python.org/pep-0723/) metadata:
...
its ecosystem has evolved. But my team at [**Buffer**](https://buffer.com) works
...
The output is self-contained. No runtime dependencies. Ship it with your skill
and Claude runs it directly.

This works, but requires a build step. You must rebuild after every change.
Debugging bundled code is harder. It's the opposite of what I wanted.
...
No installation needed. The runtime fetches the module on first use. Version
pinning lives in the URL.

The problem: Node.js doesn't natively support HTTPS imports without experimental
flags or custom loaders. Some packages don't work well as pure ESM. Network
latency on every cold start adds up.

## Approach 3: Google `zx` with `--install`

[zx](https://google.github.io/zx/) is Google's tool for writing shell scripts in
...
This gets closer to what I wanted. But version pinning through comments feels
hacky. And there's no native TypeScript support—you'd need
[tsx](https://github.com/privatenumber/tsx) or similar.

## Approach 4: Bun

[Bun](https://bun.sh) takes a different approach. Auto-install is built into the
runtime. Write normal imports and Bun handles the rest:
...
Version pinning happens directly in the import path—cleaner than **zx**'s
comment syntax. TypeScript runs natively. Startup is fast.

The catch: **Bun** might not be installed in every environment. Claude Code
environments have Node.js and npm, but not necessarily Bun.
...
`import { z } from "zod@3.0.0"`) will throw a `VersionSpecifierNotAllowedHere`
...
[Bun's Node.js compatibility documentation](https://bun.sh/docs/runtime/nodejs-apis)
before committing to this approach.

**First-run latency still exists.** The first execution downloads Bun via npx
...
environments that don't preserve Bun's cache.
...
**When to use zx instead.** If you need guaranteed Node.js compatibility—because
you're using a package that relies on Node-specific internals, or your team has
strict runtime
requirements—[zx with --install](https://google.github.io/zx/cli#install) is the
safer choice. It runs on Node.js directly, so compatibility is never a question.
The trade-off is no native TypeScript and the comment-based version pinning.

For most skills that use common packages like **lodash**, **zod**, or
**csv-parse**, Bun works fine. But know the escape hatch exists.
...
If you've used Python's **uv** and wished JavaScript had something similar, this
is it. Same philosophy, same workflow, familiar tools. And when you hit Bun's
edges, **zx** is there as a fallback.
...
- [Bun Module Resolution](https://bun.sh/docs/runtime/module-resolution) —
  Understanding how Bun resolves modules
...
- [uv](https://docs.astral.sh/uv/) — Python's package manager with inline script
  dependencies (`uv run`)
