...
date: "2025-12-22"
spoiler:
  I built granola-cli to access my AI meeting notes from the command line. Now I
  can query my meetings with Claude Code, grep transcripts, and pipe action
  items directly into my workflows.
tags:
  - typescript
  - node-js
  - cli
...
I built **granola-cli** to access my AI meeting notes from the command line. Now
I can query my meetings with **Claude Code**, grep transcripts, and pipe action
items directly into my workflows.
...
Copy-pasting from a web UI wasn't cutting it. I looked for existing solutions
and found
...
repo. The groundwork existed. What didn't exist was a proper CLI to make this
useful day-to-day.
...
Notation), a format designed specifically for LLM consumption. TOON delivers the
same structured data as JSON but uses **40% fewer tokens**.

When you pipe meeting data to Claude or another model, every token saved means
more context for your actual question. TOON makes the difference between fitting
one meeting or three into your context window.
...
secrets into `~/.config/toolname/credentials.json` and call it a day. One
accidental `git add .` or a misconfigured backup, and your tokens are exposed.

The CLI uses **cross-keychain** to store credentials in your OS's native
credential manager—macOS Keychain, Windows Credential Manager, or Linux Secret
Service. These systems encrypt secrets at rest, integrate with your login
session, and follow platform security best practices. Your Granola tokens never
touch the filesystem in readable form.

### Token Refresh with File Locking

Granola uses single-use refresh tokens—each token can only be used once before
it's invalidated. This improves security but creates a race condition: if two
CLI processes refresh simultaneously, one gets a valid token while the other
wastes the refresh token and fails.

The CLI solves this with file-based locking. Before refreshing tokens, the
process acquires an exclusive lock on a file in your temp directory. If another
process is already refreshing, the second one waits (with a 30-second timeout)
rather than racing. The lock is released immediately after the refresh
completes, so parallel CLI invocations work smoothly—they just take turns
...
The CLI walks this tree and converts it to Markdown. Headings become `#` lines,
lists become `-` items, and text marks become their Markdown equivalents: bold
wraps in `**`, italic in `*`, code in backticks. The conversion preserves nested
structures, so a bulleted list inside a blockquote renders correctly. You get
readable Markdown you can pipe to other tools, search with grep, or feed to an
LLM.

### Natural Language Date Parsing

Nobody wants to type ISO dates. The CLI accepts "today", "yesterday", "3 days
...
Monday?), and always returns UTC timestamps that match Granola's API
expectations. It makes the common case—"show me yesterday's meetings"—a single
intuitive flag.
