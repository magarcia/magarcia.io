...
date: "2026-01-15"
spoiler: "L'autoinstal·lació de Bun es trenca quan existeix qualsevol directori node_modules en les rutes pares—un problema per a skills portables. L'especificador npm: de Deno funciona a tot arreu, des de directoris de projecte fins a monorepos, sense sorpreses."
tags:
  - typescript
  - deno
  - bun
...
La setmana passada vaig escriure sobre [utilitzar npx bun per escriure skills de Claude Code](/ca/writing-powerful-claude-code-skills-with-npx-bun) amb dependències de tercers. L'enfocament funcionava per a casos simples—scripts autocontinguts amb paquets que s'autoinstal·len, sense pas de compilació. Però després d'utilitzar-lo en entorns reals, vaig descobrir un problema significatiu.
...
El crèdit per la solució és per a [J Edward Wynia](https://www.threads.net/@jwynia/post/DTgiB62DaxN), qui em va assenyalar cap a Deno en resposta a aquell article. Vaig oblidar per què em vaig saltar Deno inicialment—probablement perquè la sintaxi de Bun semblava més neta—però el suggeriment era correcte.
...
L'estructura segueix el mateix patró de l'[article original](/ca/writing-powerful-claude-code-skills-with-npx-bun)—un `SKILL.md` apuntant a scripts executables. Els únics canvis són el shebang i les APIs específiques de Deno:
...
- [Escrivint Potents Skills de Claude Code amb npx bun](/ca/writing-powerful-claude-code-skills-with-npx-bun) — L'exploració original d'aquest enfocament
- [Deno — Un runtime modern per a JavaScript i TypeScript](https://deno.land)
- [Compatibilitat npm de Deno](https://docs.deno.com/runtime/fundamentals/node/#using-npm-packages) — Com funciona l'especificador `npm:`
- [Documentació d'Autoinstal·lació de Bun](https://bun.sh/docs/runtime/auto-install) — Entenent quan s'activa l'autoinstal·lació
