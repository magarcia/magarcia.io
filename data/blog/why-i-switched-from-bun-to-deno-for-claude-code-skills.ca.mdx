...
spoiler:
  ...
  node_modules en les rutes pares—un problema per a skills portables.
  L'especificador npm: de Deno funciona a tot arreu, des de directoris de
  projecte fins a monorepos, sense sorpreses."
tags:
  - typescript
  - deno
  - bun
...
La setmana passada vaig escriure sobre
[utilitzar npx bun per escriure skills de Claude Code](/ca/writing-powerful-claude-code-skills-with-npx-bun)
amb dependències de tercers. L'enfocament funcionava per a casos simples—scripts
autocontinguts amb paquets que s'autoinstal·len, sense pas de compilació. Però
després d'utilitzar-lo en entorns reals, vaig descobrir un problema
significatiu.
...
resolució—encara que necessites la flag `--node-modules-dir=false` per
assegurar aquest comportament quan executes des de directoris que ja tenen una
carpeta `node_modules`. Comportament consistent a tot arreu.
