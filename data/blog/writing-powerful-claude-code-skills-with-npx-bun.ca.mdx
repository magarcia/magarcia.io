...
date: "2026-01-07"
updated: "2026-01-15"
spoiler: "Com escriure skills de Claude Code que utilitzin paquets npm de tercers sense un pas de compilació. Utilitzar npx bun et proporciona dependències auto-instal·lables, TypeScript natiu i una fixació de versions neta—l'equivalent en JavaScript a l'uv run de Python."
tags:
  - typescript
  - node-js
  - bun
...
> **Actualització (Gener 2026):** Després de més proves, he descobert que la detecció de `node_modules` de Bun pot trencar la instal·lació automàtica en situacions inesperades. Ara recomano Deno per a aquest cas d'ús. L'enfocament que es descriu a continuació encara funciona per a scripts independents, però consulta [Per què vaig canviar de Bun a Deno per a Skills de Claude Code](/ca/why-i-switched-from-bun-to-deno-for-claude-code-skills) per a l'explicació completa.

Volia construir skills de [Claude Code](https://docs.anthropic.com/en/docs/claude-code/skills) més complexes. Claude Code és la CLI d'Anthropic per a la programació amb agents—i les skills et permeten estendre-la amb instruccions personalitzades i scripts executables. No només markdown, sinó scripts que utilitzen biblioteques de tercers: analitzar fitxers CSV, manipular fulls de càlcul, cridar API amb clients HTTP adequats.
...
Python resol això de manera elegant amb [uv](https://docs.astral.sh/uv/). Declares les dependències en línia utilitzant metadades [PEP 723](https://peps.python.org/pep-0723/):
...
Python va ser el meu primer llenguatge de programació professional, i encara admiro com ha evolucionat el seu ecosistema. Però el meu equip a [**Buffer**](https://buffer.com) treballa en TypeScript. Necessitava alguna cosa que fos fàcil d'aprendre per als meus companys—paquets npm familiars, sintaxi familiar—però tan flexible i potent com `uv run`.
...
[zx](https://google.github.io/zx/) és l'eina de Google per escriure scripts de shell en JavaScript. Embolcalla `child_process` i afegeix comoditats com el literal de plantilla `$` per executar comandes.
...
Això s'acosta més al que volia. Però la fixació de versions a través de comentaris sembla un pedaç. I no hi ha suport natiu per a TypeScript—necessitaries [tsx](https://github.com/privatenumber/tsx) o similar.

## Enfocament 4: Bun

[Bun](https://bun.sh) pren un enfocament diferent. L'auto-instal·lació està integrada en el temps d'execució. Escriu importacions normals i Bun s'encarrega de la resta:
...
**Quan utilitzar zx en el seu lloc.** Si necessites compatibilitat garantida amb Node.js—perquè estàs utilitzant un paquet que depèn de components interns específics de Node, o el teu equip té requisits estrictes de temps d'execució—[zx amb --install](https://google.github.io/zx/cli#install) és l'opció més segura. S'executa directament a Node.js, de manera que la compatibilitat mai és una qüestió. La contrapartida és que no hi ha TypeScript natiu i la fixació de versions basada en comentaris.
...
[^1]: [Documentació d'Auto-Install de Bun](https://bun.sh/docs/runtime/auto-install) — "If no node_modules directory is found in the working directory or higher, Bun will abandon Node.js-style module resolution in favor of the Bun module resolution algorithm."

- [Documentació de Skills de Claude Code](https://docs.anthropic.com/en/docs/claude-code/skills)
- [Bun](https://bun.sh) — El temps d'execució de JavaScript amb auto-instal·lació integrada
- [Resolució de Mòduls de Bun](https://bun.sh/docs/runtime/module-resolution) — Com Bun resol mòduls
- [Google zx](https://google.github.io/zx/) — Una eina per escriure millors scripts
- [esm.sh](https://esm.sh) — paquets npm com a mòduls ES a través de CDN
- [uv](https://docs.astral.sh/uv/) — Gestor de paquets de Python amb dependències d'scripts en línia (`uv run`)
- [PEP 723](https://peps.python.org/pep-0723/) — Especificació de metadades d'scripts en línia per a Python
