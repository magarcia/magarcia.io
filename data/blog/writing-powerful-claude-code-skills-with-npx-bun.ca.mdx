...
updated: "2026-01-15"
spoiler:
  "Aprèn com escriure skills de Claude Code amb paquets npm de tercers i sense
  pas de compilació. Utilitzar npx bun proporciona instal·lació automàtica de
  dependències, suport natiu de TypeScript i fixació neta de versions en
  imports—l'equivalent en JavaScript a l'uv run de Python per a scripts
  autocontinguts."
tags:
  - typescript
  - node-js
...
Les **skills de Claude Code** et permeten estendre la CLI de codificació amb
agents d'Anthropic amb instruccions personalitzades i scripts executables. Però,
què passa si el teu script necessita paquets npm de tercers com **lodash**,
**zod** o **csv-parse**? Sense un pas de compilació ni carpeta `node_modules`,
les importacions fallen. Aquesta guia mostra com utilitzar `npx bun` per
escriure skills TypeScript autocontinguts que autoinstal·len dependències en
temps d'execució—sense necessitat de `package.json`.

El problema: les
[skills de Claude Code](https://docs.anthropic.com/en/docs/claude-code/skills)
no tenen un pas de compilació. No pots simplement fer
