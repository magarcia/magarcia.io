---
title: "YumML: Un Formato de Recetas YAML Legible para la Era de la IA"
date: "2026-01-10"
spoiler:
  YumML es un formato de recetas de codigo abierto basado en YAML que es legible
  por humanos, procesable por maquinas y optimizado para asistentes de IA.
  Descubre por que los formatos de recetas existentes fallan y como YumML
...
**YumML** es un formato de recetas legible por humanos basado en YAML que hace
las recetas de cocina faciles de leer, escribir y procesar. A diferencia de
formatos basados en XML como RecipeML o formatos propietarios, YumML prioriza la
legibilidad mientras permanece completamente procesable por maquinas y amigable
con la IA.

Hace anos, hablando con un amigo, nos dimos cuenta de que no existe un formato
estandar para guardar recetas de cocina. En ese momento estaba aprendiendo
React, y uno de mis primeros proyectos fue una app de gestión de recetas. No
tuve éxito (como tantos proyectos que empecé y nunca terminé), pero en el camino
encontré un borrador de especificación para un formato de recetas: **YumML**.

<br />

![Comic XKCD 927: Una persona explica que hay 14 estandares en competencia, asi que crean uno nuevo universal. Resultado: 15 estandares en competencia.](https://imgs.xkcd.com/comics/standards.png)

_Sí, soy plenamente consciente de la ironía.
[xkcd.com/927](https://xkcd.com/927)_

<br />

Paul Jenkins publicó este formato en [vikingco.de](http://vikingco.de/), pero el
sitio ya no está disponible. Las únicas referencias que quedan son del
[Internet Archive](https://web.archive.org/web/20160730232450/http://vikingco.de/)
y del
[repositorio de la página](https://github.com/vikingcode/vikingcode.github.io),
que todavía sigue accesible.

> Las palabras clave "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT",
> "SHOULD", "SHOULD NOT", "RECOMMENDED", "MAY" y "OPTIONAL" en este documento
> deben interpretarse como se describe en
> [RFC 2119](https://tools.ietf.org/html/rfc2119).

## Historia

El primer borrador del formato YumML es del 21 de septiembre de 2011, creado por
Paul Jenkins. Puedes revisar la
[especificación original aquí](https://web.archive.org/web/20160730232450/http://vikingco.de/yumml.html),
...
recetas de cocina y, siendo sincero, la mayoría son bastante malos. El panorama
de formatos de intercambio de recetas está fragmentado, pero casi todos
comparten un problema común: la falta de legibilidad humana.

La mayoría del software de cocina usa formatos propietarios que solo ese
software puede leer. Algunos pueden importarse a otros programas.
...
[enorme colección de recetas](http://www.ffts.com/recipes.htm) disponible en
línea.
...
Los microdatos HTML como
[Google rich snippets](https://developers.google.com/search/docs/data-types/recipe)
y los [microdatos de schema.org](http://schema.org/Recipe) son ampliamente
usados por sitios comerciales de recetas. Aunque el objetivo principal de los
microdatos es la legibilidad por máquinas (especialmente para SEO), la gente
sigue leyendo y siguiendo las recetas.
...
- Busca ser algo así como el _markdown_ de las recetas (pero que siga siendo
  fácil de procesar por software).

YumML está basado en YAML para crear un formato legible tanto por humanos _como_
por sistemas.
...
Investigaciones recientes muestran que el formato de datos impacta
significativamente tanto la **eficiencia de tokens** (coste) como la **precisión
del modelo** cuando los LLMs procesan información estructurada.

[Comparativas entre diferentes modelos](https://www.improvingagents.com/blog/best-nested-data-format/)
muestran que YAML usa entre un **27-40% menos tokens que JSON** y un **38-40%
menos que XML** para los mismos datos. Más allá del ahorro de costes, el formato
también afecta la comprensión: YAML logró una **precisión entre 12-18 puntos
porcentuales mayor** que JSON cuando los modelos extraían información de datos
anidados. La sintaxis más limpia con menos ruido de puntuación ayuda a los
modelos a procesar el contenido semántico de forma más fiable. Aunque
[JSON minificado puede ser más eficiente](https://www.curiouslychase.com/posts/yaml-vs-json-for-llm-token-efficiency-the-minification-truth),
sacrifica por completo la legibilidad humana, lo cual va en contra de los
objetivos fundamentales de YumML.
...
- **Humanos** pueden leerlo sin entrenamiento (sin corchetes angulares ni
  llaves)
- **Máquinas** pueden procesarlo con bibliotecas estándar en cualquier lenguaje
- **Modelos de IA** lo procesan de forma más eficiente y precisa que las
  alternativas

Esto hace que YumML sea particularmente adecuado para aplicaciones modernas de
recetas donde una IA podría ayudarte a escalar una receta, sugerir
sustituciones, o convertir entre sistema métrico e imperial—todo mientras
mantiene el formato fuente legible en un editor de texto.
...
- **DEBE** ser autocontenido, por lo que **NO DEBE** requerir recursos
  adicionales para ser interpretado.
- **DEBE** tener soporte para diferentes sistemas de medidas _(métrico,
  imperial)_.
- **DEBERÍA** ser fácil de traducir a diferentes idiomas _(las recetas tienen
  una fuerte influencia cultural y el idioma no debería ser una barrera para
  quien quiera entenderlas)_.
...
Hay tres secciones principales que toda receta **DEBE** incluir: la cabecera, la
lista de ingredientes y las instrucciones.
...
**Formato de duración**: Cadenas legibles por humanos. Para máxima
compatibilidad, usa números enteros seguidos de `minutes`, `hours` o `seconds`
(ej., `15 minutes`, `1 hour 30 minutes`). Los procesadores **DEBERÍAN** también
aceptar abreviaciones comunes (`15 min`, `1 hr`) y variaciones naturales
(`1.5 hours`).
...
La sección `ingredients` es una lista **REQUERIDA** de todos los ingredientes
...
Para soportar la conversión entre sistemas métrico e imperial, las
implementaciones **DEBERÍAN** reconocer estas abreviaciones de unidades
canónicas:
...
| Peso      | `g`, `kg`, `oz`, `lb`                                  |
| Conteo    | _(omitir `unit` para elementos contables como huevos)_ |
...
La sección `instructions` es una lista **REQUERIDA** de pasos para preparar la
receta. Las instrucciones soportan dos formatos: una lista simple plana o
secciones agrupadas para recetas complejas.
...
Para recetas complejas con múltiples etapas, las instrucciones **PUEDEN**
organizarse en secciones:

> **Nota de implementación**: Los procesadores **DEBEN** manejar ambos formatos.
> Verificar la presencia de `section` para determinar el formato: si algún
> elemento tiene un campo `section`, tratar como agrupado; de lo contrario,
> tratar como simple.
...
Cocinar es impreciso. Las recetas reales incluyen instrucciones como "sal al
gusto", "una pizca de nuez moscada" o "mantequilla para engrasar". Forzar una
cantidad numérica excluiría estos casos o llevaría a los autores a soluciones
forzadas como `quantity: 0`.
...
La especificación recomienda un subconjunto estructurado (`entero + unidad`)
para herramientas que necesitan un procesamiento fiable, pero mantiene la
flexibilidad suficiente para aceptar variaciones naturales.
...
Esta especificación es un punto de partida—para ser útil, YumML necesita
implementaciones. Si te interesa contribuir:

- **Escribe un parser** en tu lenguaje favorito (TypeScript, Python, Go,
  Rust...)
- **Crea un convertidor** de [schema.org/Recipe](http://schema.org/Recipe) a
  YumML
- **Desarrolla una extensión para VS Code** con resaltado de sintaxis y
  validación
- **Experimenta con LLMs** para ver qué tan bien pueden generar y procesar YumML

Si construyes algo, me encantaria saberlo.

---

_¿Interesado en mas proyectos de codigo abierto? Echa un vistazo a
[cross-keychain](/es/cross-platform-secret-storage-with-cross-keychain), una
libreria de almacenamiento de secretos multiplataforma, o mira como use IA para
disenar el [protocolo QWBP](/es/air-gapped-webrtc-breaking-the-qr-limit) para
WebRTC sin servidor._
