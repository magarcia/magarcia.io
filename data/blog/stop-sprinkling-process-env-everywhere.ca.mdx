...
spoiler:
  Vaig llançar
  [`env-interpolation`](https://www.npmjs.com/package/env-interpolation), un
  petit ajudant de TS que interpola `${VAR}` i `${VAR:default}` dins de valors
  de configuració. Recorre cadenes en objectes/arrays i mai toca les claus, per
  la qual cosa les formes es mantenen estables i predictibles—molt orientat a
  configuració.
tags:
  - typescript
  - node-js
...
Vaig llançar **env-interpolation**, un petit ajudant de TS que interpola
`${VAR}` i `${VAR:default}` dins de **valors de configuració**. Recorre cadenes
en objectes/arrays i **mai toca les claus**, per la qual cosa les formes es
mantenen estables i predictibles—molt orientat a configuració.

Nascut de **mcp-tool-selector**, on necessitava configuració en capes sense
filtrar secrets o omplir de crides a `process.env`. Es va convertir en una
petita utilitat afilada, així que la vaig publicar.
...
múltiples agents d'IA (Claude, Copilot, Gemini & Codex). Si les teves
configuracions abasten fitxers, entorns i eines, això hauria d'eliminar algunes
molèsties.
