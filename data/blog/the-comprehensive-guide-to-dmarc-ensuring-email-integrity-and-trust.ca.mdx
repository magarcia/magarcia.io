---
title: "Una Guia Completa sobre DMARC: Assegurant la Integritat i Confiança del Correu Electrònic"
date: "2023-09-11"
spoiler: En construir un projecte digital, assegurar que els teus correus electrònics arribin de manera segura i siguin de confiança és primordial. Però com navegues el laberint de la seguretat del correu electrònic? Entra DMARC. Juntament amb els seus companys, SPF i DKIM, aprofundim en establir la integritat del correu electrònic i combatre amenaces com la suplantació d'identitat i el phishing.
tags:
  - buildinpublic
  - learninpublic
---

En els primers dies de desenvolupar [Voices.ink](https://voices.ink/), un projecte col·laboratiu amb [@Ester Martí](https://github.com/estermv) que transcriu sense esforç notes de veu al teu compte de Notion, vaig enfrontar un dubte comú. Com podia assegurar que els nostres correus electrònics transaccionals no acabessin perduts en el buit de les carpetes d'spam o pitjor, fossin utilitzats com a arma per a phishing? La meva investigació per a un entorn de correu electrònic segur i de confiança em va portar a DMARC.
...
![Diagrama de Com Funciona DMARC](https://www.globalcyberalliance.org/wp-content/uploads/GCA-DMARC-Resource-Kit-Infographic-update@2x@0.5x.png)
_Crèdit de la Imatge: Global Cyber Alliance_
...
Aquest registre es tradueix a: "Si un correu electrònic falla l'autenticació DMARC, rebutja'l. I envia reports agregats de DMARC a [reports@example.com](mailto:reports@example.com)."
...
- `rua=mailto:reports@example.com`: Adreça on s'han d'enviar els reports agregats de DMARC.
- `ruf=mailto:forensic@example.com`: Adreça on s'han d'enviar els reports forenses (detallats) de DMARC.
...
- **Guia DMARC**: Una guia completa per la Global Cyber Alliance que cobreix els matisos de DMARC en detall. [Mira-la aquí](https://www.globalcyberalliance.org/dmarc/).
- **Verificador de Configuració DMARC**: Una eina inavaluable proporcionada per la Global Cyber Alliance. No només verifica si el teu DMARC està configurat correctament, sinó que també ofereix consells sobre rectificacions si és necessari. [Prova l'eina aquí](https://dmarcguide.globalcyberalliance.org/#/).
