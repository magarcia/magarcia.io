...
updated: "2026-01-15"
spoiler:
  "Cómo escribir skills de Claude Code que utilizan paquetes npm de terceros sin
  un paso de compilación. Usar npx bun te proporciona instalación automática de
  dependencias, TypeScript nativo y un fijado de versiones limpio: el
  equivalente en JavaScript al uv run de Python."
tags:
  - typescript
  - node-js
  - bun
...
> **Actualización (15 de enero de 2026):** Después de más pruebas, he
> descubierto que la detección de `node_modules` de Bun puede romper la
> instalación automática en situaciones inesperadas. Ahora recomiendo Deno para
> este caso de uso. El enfoque que se describe a continuación sigue funcionando
> para scripts independientes, pero consulta
> [Por qué cambié de Bun a Deno para Skills de Claude Code](/es/why-i-switched-from-bun-to-deno-for-claude-code-skills)
> para la explicación completa.

Quería construir skills de
[Claude Code](https://docs.anthropic.com/en/docs/claude-code/skills) más
complejas. Claude Code es la CLI de Anthropic para programación agéntica—y las
skills te permiten extenderla con instrucciones personalizadas y scripts
ejecutables. No solo markdown, sino scripts que usan librerías de terceros:
parsear archivos CSV, manipular hojas de cálculo, llamar a APIs con clientes
HTTP adecuados.

El problema: las skills no tienen paso de compilación (_build step_). No puedes
simplemente hacer `import lodash from 'lodash'` y esperar que funcione. El
script se ejecuta en un entorno limpio sin carpeta `node_modules`.

## Lo que quería: uv run para JavaScript

Python resuelve esto elegantemente con [uv](https://docs.astral.sh/uv/).
Declaras las dependencias en línea (_inline_) usando metadatos
...
[**Buffer**](https://buffer.com) trabaja en TypeScript. Necesitaba algo que
...
La salida es autocontenida. Sin dependencias en tiempo de ejecución. Envíalo con
tu skill y Claude lo ejecuta directamente.

Esto funciona, pero requiere un paso de compilación. Debes reconstruir después
de cada cambio. Depurar código empaquetado es más difícil. Es lo opuesto a lo
que quería.
...
No requiere instalación. El entorno de ejecución (_runtime_) obtiene el módulo
en el primer uso. La versión fijada vive en la URL.

El problema: Node.js no soporta nativamente importaciones HTTPS sin _flags_
experimentales o _loaders_ personalizados. Algunos paquetes no funcionan bien
como ESM puro. La latencia de red en cada arranque en frío se acumula.

## Enfoque 3: Google zx con --install

[zx](https://google.github.io/zx/) es la herramienta de Google para escribir
...
Esto se acerca más a lo que quería. Pero fijar versiones a través de comentarios
se siente como un parche. Y no hay soporte nativo de TypeScript—necesitarías
[tsx](https://github.com/privatenumber/tsx) o similar.

## Enfoque 4: Bun

[Bun](https://bun.sh) toma un enfoque diferente. La auto-instalación está
integrada en el _runtime_. Escribe importaciones normales y Bun maneja el resto:
...
La fijación de versiones ocurre directamente en la ruta de importación—más
limpio que la sintaxis de comentarios de **zx**. TypeScript se ejecuta
nativamente. El arranque es rápido.

La trampa: Puede que **Bun** no esté instalado en todos los entornos. Los
entornos de Claude Code tienen Node.js y npm, pero no necesariamente Bun.
...
Ahora tienes scripts TypeScript autocontenidos—sin invocación explícita
necesaria.
...
`import { z } from "zod@3.0.0"`) arrojarán un error
...
(_edge-case_) de Node.js—ciertas operaciones de `fs`, opciones específicas de
`child_process`, addons nativos—podrías encontrar problemas inesperados. Revisa
la
[documentación de compatibilidad de Bun con Node.js](https://bun.sh/docs/runtime/nodejs-apis)
antes de comprometerte con este enfoque.

**La latencia de la primera ejecución existe.** La primera ejecución descarga
Bun vía npx (~100MB dependiendo de la arquitectura) e instala dependencias. En
una conexión lenta o en un entorno de arranque en frío, esto añade un tiempo
notable. Las ejecuciones subsiguientes son rápidas, pero ese impacto inicial
importa si tu skill se ejecuta en entornos efímeros que no preservan la caché de
Bun.
...
en línea (_inline_) en la skill desplegada.

**Cuándo usar zx en su lugar.** Si necesitas compatibilidad garantizada con
Node.js—porque estás usando un paquete que depende de internos específicos de
Node, o tu equipo tiene requisitos estrictos de
_runtime_—[zx con --install](https://google.github.io/zx/cli#install) es la
opción más segura. Se ejecuta en Node.js directamente, así que la compatibilidad
nunca es una duda. El compromiso es no tener TypeScript nativo y la fijación de
versiones basada en comentarios.

Para la mayoría de skills que usan paquetes comunes como **lodash**, **zod** o
**csv-parse**, Bun funciona bien. Pero ten en cuenta que existe esa vía de
escape.
...
Si has usado **uv** de Python y deseabas que JavaScript tuviera algo similar, es
esto. Misma filosofía, mismo flujo de trabajo, herramientas familiares. Y cuando
llegues a los límites de Bun, **zx** está ahí como respaldo.

## Referencias

[^1]:
    [Documentación de Auto-Install de Bun](https://bun.sh/docs/runtime/auto-install)
...
- [Bun](https://bun.sh) — El runtime de JavaScript con auto-instalación
  integrada
- [Resolución de Módulos de Bun](https://bun.sh/docs/runtime/module-resolution)
  — Entendiendo cómo Bun resuelve módulos
- [Google zx](https://google.github.io/zx/) — Una herramienta para escribir
  mejores scripts
- [esm.sh](https://esm.sh) — Paquetes npm como módulos ES a través de CDN
- [uv](https://docs.astral.sh/uv/) — Gestor de paquetes de Python con
  dependencias de script en línea (`uv run`)
- [PEP 723](https://peps.python.org/pep-0723/) — Especificación de metadatos de
  script en línea para Python
